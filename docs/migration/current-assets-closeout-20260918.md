...
没有发现可以安全删除而不改变生产语义的 `latest` 入口，因此不删除这些入口；它们不是旧仓库迁移残留，而是当前数据平台仍在使用的操作契约。
...
本轮不伪造清单、不把不完整目录发布为当前资产，也不让晨报/晚报依赖它。该项是外部数据源阻塞，不是仓库迁移遗漏。
...
- `latest` 兼容入口：完成审计，按消费者保留；不是无主残留。
